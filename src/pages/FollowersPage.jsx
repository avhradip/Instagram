...
function FollowersPage() {
    ...
    const { followings, followers, user, loading, buttonloading } = useSelector((state) => state?.user)

    const data = querie.length > 0 ? results : followers;
                            ...
                            data.map(item => (
                                <div key={item._id}>
                                    <div className='flex items-center justify-between p-2 '>
                                        <div className='flex items-center gap-2 '>
                                            ...
                                            <div>
                                                <div className='flex items-center gap-2'>
                                                    ...
                                                    <p className='text-[12px] font-semibold' onClick={() => alert(item._id)}>{item?.userName}</p>


                                                    {!followings?.some(f => f._id?.toString() === item._id) && (
                                                        <button
                                                            onClick={() => followFun({ id: user._id, followerId: item._id })}
                                                            className='text-blue-500 text-[12px] font-medium flex items-center gap-1 hover:underline'
                                                        >
                                        ...
                                        {user._id === id &&
                                            <button className='bg-gray-200 px-4 py-2 rounded-[10px] text-sm font-medium' onClick={() => removefollowerFun({ id: user?._id, followerId: item._id })}>
                                                {buttonloading ? <Loader2 className='w-6 h-6 animate-spin' /> : "Remove"}
                                            </button>
                                        }
                                    </div>
                                </div>
